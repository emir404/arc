...
function deriveWhite(hex: string): string {
  const [r, g, b] = hexToRgb(hex);
  const [h, s] = rgbToHsl(r, g, b);
  const newS = Math.min(s * 0.33, 0.33);
  return hslToHex(h, newS, 0.988);
}
...
/** Returns the brand color if it has enough contrast against bg, otherwise black */
function previewTextColor(brandHex: string, bgHex: string): string {
  return contrastRatio(brandHex, bgHex) >= 3 ? brandHex : "#000000";
}
...
export function FindYourWhite() {
  const [brandColor, setBrandColor] = useState("#1a1a2e");
  const [inputValue, setInputValue] = useState("#1a1a2e");
  const [format, setFormat] = useState<ColorFormat>("hex");
  const inputRef = useRef<HTMLInputElement>(null);

  const yourWhite = deriveWhite(brandColor);
  const brandContrast = getContrastColor(brandColor);

  const brandFormatted = formatColor(brandColor, format);
  const whiteFormatted = formatColor(yourWhite, format);
  ...
  return (
    <section className="min-h-[calc(100dvh-8rem)] px-5 md:px-12 lg:px-24 xl:px-32 pt-4 md:pt-8 pb-16 md:pb-24">
      ...
      >
        Pure #fff is sterile. Derive a near-white tinted with your brand
        color&rsquo;s hue.
      </motion.p>
      ...
      <div className="flex flex-col xl:flex-row gap-10 xl:gap-8">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.div
              className="flex-1 min-w-0 rounded-xl p-4 flex flex-col justify-between aspect-[3/2]"
              style={{ backgroundColor: yourWhite }}
              animate={{ backgroundColor: yourWhite }}
              transition={{ duration: 0.3 }}
            >
              <span className="text-[10px] font-medium uppercase tracking-widest text-black/40">
                Your White
              </span>
              <CopyButton value={whiteFormatted} variant="light" />
            </motion.div>
          </div>
        </motion.div>
        ...
        >
          {/* With pure #ffffff */}
          <div
            className="rounded-2xl p-6 md:p-8 flex-1 flex flex-col justify-between bg-white border border-black/5 overflow-hidden"
          >
            <p className="text-[10px] font-medium uppercase tracking-widest text-black/30 mb-5">
              With #ffffff
            </p>
            <motion.p
              className="text-2xl md:text-3xl font-medium leading-[1.2] break-words"
              style={{ color: previewTextColor(brandColor, "#ffffff") }}
              animate={{ color: previewTextColor(brandColor, "#ffffff") }}
              transition={{ duration: 0.3 }}
            >
          ...
          {/* With your white */}
          <motion.div
            className="rounded-2xl p-6 md:p-8 flex-1 flex flex-col justify-between border border-black/5 overflow-hidden"
            style={{ backgroundColor: yourWhite }}
            animate={{ backgroundColor: yourWhite }}
            transition={{ duration: 0.3 }}
          >
            ...
            <motion.p
              className="text-2xl md:text-3xl font-medium leading-[1.2] break-words"
              style={{ color: previewTextColor(brandColor, yourWhite) }}
              animate={{ color: previewTextColor(brandColor, yourWhite) }}
              transition={{ duration: 0.3 }}
            >
              The quick brown fox jumps over the lazy dog.
            </motion.p>
